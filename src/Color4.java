...
//			this.slopes.putAll(slopes);
	...
	private static long dp[]=new long[3001+1];
...
//			
			...
			if(list.size()==1) {
				...
				long finalTrianglesReduced=knapsack(list,list.size()-1,k,dp);
//				//if(finalTrianglesReduced==Long.MIN_VALUE) {
//					finalTrianglesReduced=0;
//				}
	...
	private static long knapsack(List<Node> list,int index,int k,long dp[][][]) {
		...
		for(int i=0;i<list.size();++i) {
			cur=list.get(i);
			if(cur.cost<=k) {
				if( cur.count>=3 && cur.slopes.size()>=3) {
					long costCur=getCurrentProfit(cur);
					int tempF=cur.count;
					int edgeDelete=findSmallestNumSlope(cur);
					
					if(list.get(i).slopes.get(edgeDelete)==1) {
						list.get(i).slopes.remove(edgeDelete);
					}
					else {
						list.get(i).slopes.put(edgeDelete,list.get(i).slopes.get(edgeDelete)-1);
					}
					list.get(i).count=tempF-1;
					long sub=knapsack(list,i,k-cur.cost,dp);
					list.get(i).count=tempF;
					if(list.get(i).slopes.containsKey(edgeDelete)) {
						list.get(i).slopes.put(edgeDelete,list.get(i).slopes.get(edgeDelete)+1);
					}
					else {
						list.get(i).slopes.put(edgeDelete,1);
					}
					if(sub+costCur>res) {
						res=sub+costCur;
					}
