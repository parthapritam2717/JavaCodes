...
class Test3 {
	...
	private static int getCost(int family[],int k) {
		...
		if(k==1)
			...
			return Math.min(res1, totalCost);
			
		}
	}
	
	
	private static int getResult(int []family,int k) {
		int hmR[]=new int[101];
		int hmL[]=new int[101];
		int count=0;
		...
		for(int i=1;i<family.length;++i) {
			...
			int cost=getDuplicatesCount(hmL)+getDuplicatesCount(hmR)+2*k;
			if(cost<=count) {
				count=cost;
			}
		}
	...
	private static int getDuplicatesCount(int hmR[]) {
		int c=0;
		for(int i=0;i<=100;++i) {
			if(hmR[i]>1) {
				c+=hmR[i];
