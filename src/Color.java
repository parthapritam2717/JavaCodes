...
//			this.slopes.putAll(slopes);
	...
	private static void solve() throws IOException {
		...
		while(t>0) {
			...
			List<Node> list=new ArrayList<Color.Node>(c+1);
...
//				);
	...
	//private static long knapsack(List<Node> list,int index,int k,long dp[][]) {
		...
		private static long knapsack(List<Node> list,int index,int k,HashMap<String,Long> dp) {
		...
		String key=Integer.toString(index)+','+Integer.toString(k)+','+Integer.toString(tempF);
		
//		if(dp[index][k]!=-1)
//			//return dp[index][k];
//			System.out.println("Previos data found="+index+","+k+","+cur.count+"="+dp[index][k]);
//			//return dp[index][k][cur.count];
		
		if(dp.containsKey(key))
			//System.out.println("Previos data found="+index+","+k+","+cur.count+"="+dp.get(key));
			return dp.get(key);
		
		if(cur.cost>k || cur.count<3 || cur.slopes.size()<3) {
//			dp[index][k]=knapsack(list,index-1,k,dp);
//			return dp[index][k];
			long temp=knapsack(list,index-1,k,dp);
			dp.put(key, temp);
			return temp;
...
//			if(dp[index][k]!=-1)
//				
//				if(dp[index][k]!=-1 && dp[index][k]<Math.max(usedCount, unusedCount)) {
//					System.out.println("====Found a more value====!!");
//					System.out.println("Updated="+index+","+k+","+cur.count+"="+dp[index][k]+","+Math.max(usedCount, unusedCount));
//					
//				}
//				else {
//					System.out.println("Old="+index+","+k+","+cur.count+"="+dp[index][k]+","+Math.max(usedCount, unusedCount));
//				}
//			else {
//				System.out.println("New="+index+","+k+","+cur.count+"="+Math.max(usedCount, unusedCount));
//			}
			long ans=Math.max(usedCount, unusedCount);
